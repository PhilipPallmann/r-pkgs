...
* Bundles the package before checking it. This is the best practice for checking 
  ...
  in your source package, e.g. artefacts like `.so` and `.o` files which 
...
`R CMD check` returns three types of messages:
  ...
  * `NOTE`s: Mild problems. If you are submitting to CRAN, you should strive to 
    ...
    be easier. If it's not possible to eliminate a `NOTE`, you'll need to describe 
...
* __Checking for executable files__. You must not have executable files in
  ...
  security risk. Delete any executables files from your package. 
...
* __Checking installed package size__. It's easy to accidentally include large
  ...
  whole package less than 5 MB and each subdirectory is less than 1 MB. 
...
* __Checking top-level files__. Only specified files and directories are 
  ...
  To include other files, you have two choices:
    ...
    * If they need to be installed: move into `inst/`. They'll be moved back to 
...
* __Checking package dependencies__. 
    ...
    * Every package listed in `Depends` must also be imported in the 
      `NAMESPACE` or accessed with `pkg::foo`. If you don't do this, you
...
* __Checking CRAN incoming feasibility__. These checks only apply if you're 
  submitting to CRAN.
    ...
    * The `Title` and `Description` must be free from spelling mistakes. The 
      title of the package must be in title case. Neither title not description
...
* __Checking Rd `\usage` sections__.  All arguments must be documented, and 
  ...
  that you've removed, or mispelled an argument name.
...
If you use git and GitHub, as described in [git and GitHub](#git), I highly recommend learning about [Travis](https://travis-ci.org/). Travis is continuous integration service, which means that it runs automated testing code everytime you push to GitHub. For open source projects, Travis provides 50 minutes of free computation on a ubuntu server for every push. For an R package, the most useful code to run is `devtools::check()`. 
...
The Travis config is stored in yaml file called `.travis.yml`. The default config created by devtools looks like this:
...
* `./travis-tool.sh r_binary_install package`: installs an precompiled R 
...
* `WARNINGS_ARE_ERRORS=1`: make the build fail if there are any `ERROR`s or 
  any `WARNING`s. The default is to fail only on errors. I recommending using
